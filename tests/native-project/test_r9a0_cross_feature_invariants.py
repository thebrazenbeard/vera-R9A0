...
class CrossFeatureInvariantTests(unittest.TestCase):
    ...
    def test_sequence_is_not_freshness(self): self.assertTrue(self.contract["assignment_currentness"]["sequence_high_water_nonsemantic_for_freshness"])
    ...
    def test_protected_effect_never_inherits_build_pass(self):
        self.assertFalse(self.contract["assignment_currentness"]["universal_effect_eligible"])
        self.assertEqual(self.contract["feature_surface_policy"]["protected_effect_broker_confinement"], "DISABLED_OMITTED")
        self.assertFalse(self.contract["supabase"]["database_effects_enabled"])
    def test_native_compression_guard_present(self): self.assertIn("COMPRESSION_DOES_NOT_WEAKEN_AUTHORITY",self.native)
