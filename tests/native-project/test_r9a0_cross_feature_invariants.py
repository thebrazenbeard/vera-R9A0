...
class CrossFeatureInvariantTests(unittest.TestCase):
    ...
    def test_protected_effect_never_inherits_build_pass(self):
        fp=self.contract["feature_surface_policy"]
        ci=self.contract["ci"]
        self.assertEqual(fp["protected_effect_broker_confinement"], "DISABLED_OMITTED")
        self.assertIn("RELEASE_ELIGIBLE", ci["packaged_validation_report_cannot_satisfy"])
        self.assertTrue(ci["local_only_acceptance_prohibited"])
