...
- **MUNE-R9A0-FND-005:** the successor test covers role privileges, RPC authorization, ownership, forced RLS, projection correctness, stale predecessor/fork rejection, cross-thread rejection, payload limits, search-path pinning, advisory-lock presence, and migration-receipt immutability.
...
1. apply the exact pre-existing migration artifact to the authorized R9A0 build target;
2. run both transactional SQL suites plus a two-session concurrency test;
3. capture owner, policy, RLS, ACL, projection, and protected-scope readback;
4. obtain Mune exact-head review;
5. reconcile the immutable successor head with the native branch.
